...
if(!$nitCliente=="" && !$nameCliente=="" && !$apeCliente=="" && !$dirCliente=="" && !$phoneCliente=="" && !$emailCliente=="" && !$fullnameCliente==""){
    $verificar=  ejecutarSQL::consultar(" select * from cliente where NIT= '".$nitCliente."' OR Nombre= '".$nameCliente."' OR Email= '".$emailCliente."' ");
    ...
    if($verificaltotal<=0){
        ...
        echo '<img src="assets/img/error.png" class="center-all-contens"><br>La tarjeta o el nombre de usuario o el email que ha ingresado ya esta registrado.<br>Por favor ingrese otros datos';
    }
}else {
